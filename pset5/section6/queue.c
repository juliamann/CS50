...
{
    // if array capacity is full, break
    if (q.size == CAPACITY)
        return false;

    // insert new string at the end of the array
...
char* dequeue(void)
{
    ...
    // save string in current head position
    char* first = q.strings[q.head];

    // decease size and adjust head to next element in array
    q.size--;
    q.head = (q.head + 1) % CAPACITY;

    // return prior head string
    return first;
}
