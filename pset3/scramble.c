...
bool lookup(string s)
{
    ...
    // Checks first letter of user word to first letter of the middle dictionary word
    // Then halves dictionary size and recalculates dictionary values
    while (first <= last)
    {
        // First dictionary letter < first letter of user word
        if ((int)dictionary.words[middle].letters[0] < (int)s[0])
        {       
            // Half dictionary from beginning, recalculate dictionary values
            first = middle + 1;
            middle = (first + last) / 2;
        }
        // First dictionary letter > first letter of user word
        else if ((int)dictionary.words[middle].letters[0] > (int)s[0])
        {
            // Half dictionary from end, recalculate dictionary values
            last = middle - 1;
            middle = (first + last) / 2;     
        }
        // First dictionary letter == first letter of user word
        ...
        {
            // Compare next value of dictionary word to next value of user word
            bool failed = false;
            for (int i = 1; i < strlen(s); i++)
            {
                // Next dictionary letter < next letter of user word
                if ((int)dictionary.words[middle].letters[i] < (int)s[i]) 
                {
                    // Half dictionary from beginning, recalculate dictionary values
                    first = middle + 1;
                    middle = (first + last) / 2;
            ...
            // User word, s, found in dictionary and flagged as found
            if (failed == false && (strlen(s) == strlen(dictionary.words[middle].letters)) && dictionary.words[middle].found == false)
            {
                dictionary.words[middle].found = true;
                break;
            }

            // Dictionary has not found the word yet, recalculate dictionary values and loop again
            if (failed == false && (strlen(s) != strlen(dictionary.words[middle].letters)))
            {
                last = middle - 1;
                middle = (first + last) / 2;
            }
            
            // Word flagged as found - this word has been used before
            if (failed == false && (strlen(s) == strlen(dictionary.words[middle].letters)) && dictionary.words[middle].found == true)
                first = last + 1;
        }
    }
...
void scramble(void)
{
    ...
    {    
        ...
        {
            grid[x][y] = temp_grid[(DIMENSION - 1) - y][x];  
        }
    }
